...
const MegaPanel = ({ children }: { children: React.ReactNode }) => (
  ...
  >
    <div className="bg-white border-t border-gray-100 shadow-[0_24px_60px_-24px_rgba(88,60,180,0.35)]">
...
const MegaLink = ({
  ...
  return (
    <Link
      ...
      className="group flex items-start gap-3 rounded-xl p-2 -mx-2 transition-colors hover:bg-purple-50"
    >
      <span className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-500 transition-colors group-hover:border-purple-200 group-hover:text-purple-600">
        <Icon className="h-[18px] w-[18px]" strokeWidth={1.8} />
      </span>
      <span>
        <span className="block text-sm font-semibold text-gray-900 group-hover:text-purple-700">
          {item.title}
        </span>
      ...
      {byBusinessType.map((item) => {
        ...
        return (
          <Link
            ...
            href={item.href}
            onClick={onNavigate}
            className="inline-flex items-center gap-1.5 rounded-full border border-gray-200 bg-white px-3.5 py-1.5 text-sm font-medium text-gray-700 transition-colors hover:border-purple-200 hover:bg-purple-50 hover:text-purple-700"
          >
