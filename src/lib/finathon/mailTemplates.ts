  ...
  return shell(
    ...
    `
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border-radius:12px;overflow:hidden">
        <tr><td style="background:${ORANGE};padding:20px 24px;color:#ffffff;font-size:20px;font-weight:bold">Finathon 2026 — registration received</td></tr>
        <tr><td style="padding:24px">
          <p style="margin:0 0 16px">Hi ${escapeHtml(data.lead.fullName)},</p>
          <p style="margin:0 0 16px">We've received the registration for team <strong>${escapeHtml(data.teamName)}</strong>.
            Your payment is now being verified — you'll hear from us once the organisers have matched it.</p>
    ...
    `
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;text-align:left;font-size:15px;line-height:1.6">
        ...
        <tr><td style="padding:24px 0 0">
          ...
          <p style="margin:0 0 14px">Thanks for registering <strong>${escapeHtml(data.teamName)}</strong> for Finathon 2026. We've got your
            ${escapeHtml(data.amount)} payment (UTR <span style="font-family:monospace">${escapeHtml(data.utr)}</span>) and are verifying it now.
            We'll write again once it's confirmed.</p>
    ...
    `
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px">
        <tr><td style="padding:0 0 16px;text-align:left;font-size:15px;color:#1a1a1a">
          Hi ${escapeHtml(data.lead.fullName)} — your team is in the queue. Here's your pass while we verify the payment.
        </td></tr>
        <tr><td style="background:${BLUE};border-radius:14px 14px 0 0;padding:24px">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
            <tr>
              ...
              <td align="right"><span style="background:${ORANGE};color:#ffffff;font-size:11px;font-weight:bold;letter-spacing:1px;padding:5px 10px;border-radius:20px">PAYMENT UNDER REVIEW</span></td>
            </tr>
